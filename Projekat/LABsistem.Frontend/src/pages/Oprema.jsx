...
function Oprema() {
  ...
  const kabinetiZaObjekat = useMemo(() => {
    ...
    return (
      objekti.find((o) => o.id === Number(selectedObjekatID))?.kabineti || []
    );
  ...
  const kategorijeOptions = useMemo(() => {
    const unique = [
      ...new Map(opremaList.map((o) => [o.kategorija, o.kategorija])).values(),
    ];
    return unique
      .filter(Boolean)
  ...
  const filteredOprema = useMemo(() => {
    return opremaList.filter((o) => {
      ...
      const kategorijaMatch =
        !filters.kategorija || o.kategorija === filters.kategorija;
      const kabinetMatch =
  ...
  function openEditModal(oprema) {
    ...
    const obj = objekti.find((o) =>
      o.kabineti?.some((k) => k.id === oprema.kabinetID),
    );
        ...
        >
          ...
          <div style={{ position: "relative", flex: "1", minWidth: "200px" }}>
            ...
            <input
              type="text"
              name="searchTerm"
              placeholder="Pretraži (naziv, tip, serijski broj)..."
