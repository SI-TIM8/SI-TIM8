...
namespace LABsistem.Api.Validators
{
    public interface IRezervacijaValidator
    {
        ...
        Task ValidateStudentOtkazivanjeZahtjeva(int zahtjevId, int studentId);
    ...
    public class RezervacijaValidator : IRezervacijaValidator
    {
        ...
        public async Task ValidateStudentOtkazivanjeZahtjeva(int zahtjevId, int studentId)
        {
            var zahtjev = await _context.Zahtjevi
                .Include(z => z.Termin)
                .FirstOrDefaultAsync(z =>
            ...
            if (zahtjev == null)
                throw new Exception("Nemate aktivan zahtjev na čekanju za otkazivanje.");

            var terminStart = zahtjev.Termin.Datum.Date.Add(zahtjev.Termin.VrijemePocetka);
            if (terminStart <= DateTime.Now)
                throw new Exception("Zahtjev je moguće otkazati samo prije početka termina.");
        }
